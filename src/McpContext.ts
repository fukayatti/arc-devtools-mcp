...
export class McpContext implements Context {
  ...
  async newPage(): Promise<Page> {
    // [Antigravity Mod] Arc browser crashes on Target.createTarget (browser.newPage).
    // Instead of creating a new page, we reuse an existing one if available.
    const pages = await this.browser.pages();

    // 1. Try to find a blank page to reuse
    const blankPage = pages.find(p => p.url() === 'about:blank');
    if (blankPage) {
      this.selectPage(blankPage);
      return blankPage;
    }

    // 2. If no blank page, reuse the last opened page
    if (pages.length > 0) {
      const lastPage = pages[pages.length - 1];
      this.selectPage(lastPage);
      return lastPage;
    }
